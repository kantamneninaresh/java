...
# Change vaultRoleId if reqd.
#sed -i "/^\s*vaultRoleId\s*:/ s/\(vaultRoleId\).*/\1 : \"${vault_role_id}\"/" $YAML_FILE

# Change vaultSecretId if reqd.
#sed -i "/^\s*vaultSecretId\s*:/ s/\(vaultSecretId\).*/\1 : \"${vault_secret_id}\"/" $YAML_FILE
